...
func TestAdapterDecodeHapticPanelPressFromDedicatedButtonBit(t *testing.T) {
	adapter := Adapter{}
	stream := decodeAll(t, &adapter,
		rawReport(0x02, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
		rawReport(0x02, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
		rawReport(0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
		rawReport(0x02, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
	)
...
func TestAdapterDoesNotTreatPointerNoiseAsHapticPanelPress(t *testing.T) {
	adapter := Adapter{}
	stream := decodeAll(t, &adapter,
	...
	for _, event := range stream {
		if event.Gesture == "haptic_panel_press" {
			t.Fatalf("stream = %#v, want no haptic_panel_press from pointer-noise reports", stream)
		}
	}
}
...
func TestAdapterDecodeRejectsUnknownButtonBits(t *testing.T) {
	adapter := Adapter{}
	_, err := adapter.Decode(rawReport(0x02, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00))
